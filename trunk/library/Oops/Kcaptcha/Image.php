...
class Oops_Kcaptcha_Image extends Oops_Object {
	...
	function action() {
		...
		$foreground_color=array($this->config->foreground->red,$this->config->foreground->green,$this->config->foreground->blue);
		$background_color=array($this->config->background->red,$this->config->background->green,$this->config->background->blue);
		...
		while(true){
			...
			while(true) {
				...
				if(!preg_match('/cp|cb|ck|c6|c9|rn|rm|mm|co|do|cl|db|qp|qb|dp|71|11|17/', $this->keystring)) break;
			}
			...
			for($i=0; $i<$this->config->length; $i++){
				...
				$y=mt_rand( -$this->config->fluctuation_amplitude, $this->config->fluctuation_amplitude)+($this->config->height - $fontfile_height)/2+2;
						...
						for($sy=7;$sy<$fontfile_height-20;$sy+=1){
							...
							for($sx=$m['start']-1;$sx<$m['end'];$sx+=1){
								...
								if($opacity<127){
									...
									for($px=min($left,$this->config->width-1);$px>$left-12 && $px>=0;$px-=1){
		...
		$credits=empty($this->config->credits)?$_SERVER['HTTP_HOST']:$this->config->credits;
