...
abstract class Oops_Session_Abstract {
	...
	protected function setHandler() {
		if(!$this instanceof Oops_Session_Interface) {
			// @todo Consider throwing exception or error if session handler
			// does not implement required interface
			return false;
		}
		
		return session_set_save_handler(array($this, '_open'), array($this, '_close'), array(
			$this, 
			'_read'), array($this, '_write'), array($this, '_destroy'), array($this, '_gc'));
	}
